#!/usr/bin/env python3
"""Test RFD3 binder design with correct contig format."""
...
# Test with correct contig format: target + chain break + binder length
# Using backslash-zero for chain break
contig = "A2-52,\\0,40-60"
print(f"\n{'='*60}")
print(f"Testing RFD3 with contig='{contig}'")
print(f"This means: target A2-52, chain break, design 40-60 residue binder")
print('='*60)
    ...
    if result.get("status") == "completed":
        ...
        if designs:
            ...
            if pdb_content:
                ...
                # Check if binder was generated (should have ~91-111 total residues)
                if total_residues > 60:  # 51 target + some binder
                    print(f"\nSUCCESS: Binder generated!")
                    print(f"  Target: ~51 residues")
                    print(f"  Binder: ~{total_residues - 51} residues")
                else:
                    print(f"\nFAILURE: No binder generated (only {total_residues} residues)")

                # Save for inspection
                with open('binder_contig_output.pdb', 'w') as f:
                    f.write(pdb_content)
                print(f"\nSaved to binder_contig_output.pdb")
            else:
