...
# Import donor identification for chemistry-aware fixing
...
def analyze_ligand_chemistry(
    ...
    # Enhancement 2: SMILES-based H-bond donor/acceptor classification
    # Properly distinguishes donors (O-H, N-H) from acceptors (C=O, ether)
    # Run when: no template OR template didn't provide hbond_donors
    if smiles and HAS_RDKIT and (not template_info or not chem.hbond_donors):
        atom_index_map = _map_rdkit_to_pdb_names(pdb_content, ligand_res_name, smiles=smiles)
        hbond_donors, hbond_acceptors = _classify_hbond_from_smiles(smiles, atom_index_map)
        if hbond_donors or hbond_acceptors:
            # Always fill in donors from SMILES if empty
            if not chem.hbond_donors:
                chem.hbond_donors = hbond_donors
            # Only overwrite acceptors if template didn't provide them
            if not template_info or not chem.hbond_acceptors:
                chem.hbond_acceptors = hbond_acceptors
            logger.debug(
                f"SMILES H-bond classification: {len(hbond_donors)} donors, "
                f"{len(hbond_acceptors)} acceptors"
            )
...
def _map_rdkit_to_pdb_names(
    pdb_content: str,
    ligand_res_name: str = "LIG",
    smiles: Optional[str] = None,
) -> Dict[int, str]:
    """
    Build mapping from RDKit atom index to PDB atom name.

    When smiles is provided, builds the mapping using the AddHs molecule
    to correctly handle index shifts from explicit hydrogens. Maps only
    heavy atom positions to PDB HETATM names.

    When smiles is None, falls back to sequential index mapping (assumes
    PDB was generated by generate_conformer in RDKit atom order).

    Args:
        pdb_content: PDB content with HETATM records
        ligand_res_name: Residue name to filter
        smiles: SMILES string for AddHs-aware mapping

    Returns:
        Dict mapping RDKit atom index (post-AddHs) -> PDB atom name
    """
    # Collect PDB atom names in order
    pdb_names = []
    for line in pdb_content.strip().split('\n'):
        if not line.startswith('HETATM'):
            continue
    ...
    if smiles and HAS_RDKIT:
        try:
            ...
            if mol is not None:
                ...
                # Map heavy atom indices in the AddHs molecule to sequential PDB names
                mapping = {}
                pdb_idx = 0
                for i, atom in enumerate(mol_h.GetAtoms()):
                    if atom.GetAtomicNum() > 1:  # Heavy atom (not H)
                        if pdb_idx < len(pdb_names):
                            mapping[i] = pdb_names[pdb_idx]
                            pdb_idx += 1
                return mapping
        except Exception:
            pass

    # Fallback: sequential mapping (no AddHs correction)
    return {i: name for i, name in enumerate(pdb_names)}


def _classify_hbond_from_smiles(
    smiles: str,
    atom_index_map: Optional[Dict[int, str]] = None,
) -> Tuple[List[str], List[str]]:
    """
    Classify H-bond donors and acceptors from SMILES using SMARTS patterns.

    Properly distinguishes donors (atoms with H: -OH, -NH2, -SH) from
    acceptors (lone pair atoms: C=O, ether O, pyridine N).

    Args:
        smiles: SMILES string of the ligand
        atom_index_map: Optional mapping from RDKit atom index to PDB atom name.
                        If None, returns element+index names.

    Returns:
        Tuple of (donor_pdb_names, acceptor_pdb_names)
    """
    if not HAS_RDKIT:
        return ([], [])
    ...
    # Donor SMARTS: atoms that can donate H-bonds (have attached H)
    donor_smarts = {
        "hydroxyl": "[OX2H1]",
        "amine_primary": "[NX3H2]",
        "amine_secondary": "[NX3H1]",
        "thiol": "[SX2H1]",
    }

    # Acceptor SMARTS: atoms with lone pairs that accept H-bonds
    acceptor_smarts = {
        "carbonyl": "[CX3]=[OX1]",  # Match the O, not the C
        "carboxylate_o": "[OX1-][CX3]=[OX1]",
        "carboxylic_oh": "[OX2H1][CX3]=[OX1]",  # The =O is acceptor
        "ether": "[OX2]([#6])[#6]",
        "imine": "[NX2]=[CX3]",
        "pyridine": "[nX2]",
        "amide_o": "[OX1]=[CX3][NX3]",
    ...
    # Find donors
    for name, smarts_str in donor_smarts.items():
        pattern = Chem.MolFromSmarts(smarts_str)
        if pattern is None:
            continue
        for match in mol.GetSubstructMatches(pattern):
            # First atom in the match is the donor heteroatom
            donor_indices.add(match[0])

    # Find acceptors
    for name, smarts_str in acceptor_smarts.items():
        pattern = Chem.MolFromSmarts(smarts_str)
        if pattern is None:
            continue
        for match in mol.GetSubstructMatches(pattern):
            # Add O/N/S atoms from match (skip C atoms)
            for idx in match:
                atom = mol.GetAtomWithIdx(idx)
                if atom.GetSymbol() in ("O", "N", "S"):
                    acceptor_indices.add(idx)

    # Remove H atoms from donor set (we only want the heavy atoms)
    donor_indices = {i for i in donor_indices if mol.GetAtomWithIdx(i).GetSymbol() != "H"}

    def _idx_to_name(idx: int) -> str:
        if atom_index_map and idx in atom_index_map:
            return atom_index_map[idx]
        atom = mol.GetAtomWithIdx(idx)
        return f"{atom.GetSymbol()}{idx}"

    donor_names = [_idx_to_name(i) for i in sorted(donor_indices)]
    acceptor_names = [_idx_to_name(i) for i in sorted(acceptor_indices)]
...
class LigandResolver:
    ...
    def _try_template_resolution(
        ...
        # Analyze chemistry (pass SMILES for E2 H-bond donor/acceptor classification)
    ...
    def _try_pubchem_resolution(
        ...
        # Enhancement 6: Use metal-aware conformer generation when metal is present
        try:
            if result.metal_type:
                from conformer_utils import generate_metal_aware_conformer
                pdb_content = generate_metal_aware_conformer(
                    smiles,
                    metal=result.metal_type,
        ...
        # Pass SMILES for proper H-bond donor/acceptor classification (Enhancement 2)
        ...
        # 5. Enhancement 1: Wire donor identification into fixing strategy
        # Instead of blanket diffuse_all, use ligand_donors.py to find real coordination donors
        if result.metal_type and smiles and HAS_RDKIT:
            try:
                coord_mode = get_recommended_coordination_mode(
                    smiles, result.metal_type
                )
                recommended_donors = coord_mode.get("recommended_donors", [])

                if recommended_donors and result.pdb_content:
                    # Map RDKit atom indices to PDB atom names (AddHs-aware)
                    atom_index_map = _map_rdkit_to_pdb_names(
                        result.pdb_content, result.residue_code or "LIG",
                        smiles=smiles,
                    )
                    # Convert donor atom indices to PDB names
                    donor_pdb_names = []
                    for donor in recommended_donors:
                        idx = donor.get("atom_idx")
                        if idx is not None and idx in atom_index_map:
                            donor_pdb_names.append(atom_index_map[idx])

                    if donor_pdb_names:
                        result.coordination["ligand_metal_donors"] = donor_pdb_names
                        result.coordination["recommended_fixing_strategy"] = "fix_coordination"
                        result.coordination["atom_index_map"] = atom_index_map
                        result.coordination["coordination_mode"] = coord_mode.get(
                            "coordination_mode", "unknown"
                        )
                        result.coordination["compatibility_score"] = coord_mode.get(
                            "compatibility_score", 0.0
                        )
                        logger.info(
                            f"Identified {len(donor_pdb_names)} coordination donors "
                            f"for {result.name}+{result.metal_type}: {donor_pdb_names}"
                        )
                    else:
                ...
                logger.debug(f"Donor identification failed for {result.name}: {e}")
                result.coordination["recommended_fixing_strategy"] = "diffuse_all"
        else:
            # No metal or no SMILES — default to diffuse_all
            result.coordination["recommended_fixing_strategy"] = "diffuse_all"
