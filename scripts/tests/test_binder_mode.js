/**
 * Test: Binder Mode for Interface Ligand Design
 *
 * This tests Priority 3: Binder-to-target mode
 * Uses H-bond and burial conditioning for stronger binding.
 */
...
async function main() {
  console.log('=== Binder Mode Test (Priority 3) ===\n');
  console.log('Testing H-bond + burial conditioning for stronger binding.\n');
  console.log(`Generating ${NUM_DESIGNS} designs with interface_ligand="binder"...\n`);

  const startTime = Date.now();

  // Generate designs with BINDER mode
  console.log('Step 1: Generating designs with binder mode...\n');
  const rfdResult = await submitJob({
    task: 'rfd3',
    ligand_smiles: AZOBENZENE_SMILES,
    conformer_method: 'xtb',
    symmetry: { id: 'C2' },
    interface_ligand: 'binder',  // <-- BINDER MODE
    length: '80-100',
    num_timesteps: 200,
    step_scale: 1.5,
  ...
  for (let i = 0; i < designs.length; i++) {
    ...
    results.push({
      ...
      cnnScore: cnnScore,
      hbonds: hbonds,
      // Composite score: prioritize affinity, then H-bonds, then contacts
  ...
  // Summary
  console.log('=== Results Summary (Binder Mode) ===\n');
  console.log('Rank | Design | Affinity | Contacts | H-bonds | Symmetry | Valid | Score');
  console.log('-----|--------|----------|----------|---------|----------|-------|------');
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    console.log(
      `  ${i + 1}  |   ${r.index + 1}    |  ${affStr.padStart(6)}  |    ${r.geometry.totalContacts.toString().padStart(2)}    |    ${r.hbonds}    |   ${(r.geometry.symmetryScore * 100).toFixed(0).padStart(3)}%   |  ${r.geometry.isValid ? 'YES' : 'NO '}  | ${r.score.toFixed(1)}`
    );
  }

  // Best design
  const best = results[0];
  console.log('\n=== Best Design (Binder Mode) ===\n');
  ...
  if (best.affinity !== null) {
    ...
    else quality = 'NO BINDING (clashing)';
    console.log(`  Binding quality: ${quality}`);
  }

  // Save best design
  const outputPath = 'G:\\Github_local_repo\\Banta_Lab_RFdiffusion\\best_binder_design.pdb';
  fs.writeFileSync(outputPath, best.design.content);
  console.log(`\nBest design saved to: ${outputPath}`);

  // Compare with previous best (natural mode)
  console.log('\n=== Comparison with Natural Mode ===');
  console.log('Previous best (natural mode): -3.78 kcal/mol');
  console.log(`Binder mode best: ${best.affinity?.toFixed(2) || 'N/A'} kcal/mol`);
  if (best.affinity !== null && best.affinity < -3.78) {
    console.log('IMPROVEMENT! Binder mode produced better affinity.');
  } else if (best.affinity !== null && best.affinity > -3.78) {
    console.log('Natural mode was better for this run.');
  }
