#!/usr/bin/env python3
"""
Direct Round 1 execution script for Ln-citrate scaffold design.
...
OUTPUT_DIR = EXPERIMENT_DIR / "outputs" / "round_01"
...
def run_rfd3(config: dict, pdb_content: str, name: str, n_designs: int = 4) -> dict:
    ...
    print(f"  Config: contig={clean_config.get('contig')}, n_designs={n_designs}")
...
def main():
    print("="*60)
    print("Ln-Citrate Scaffold Design - Round 1")
    print("="*60)
    print(f"\nTimestamp: {datetime.now().isoformat()}")
    print(f"Output dir: {OUTPUT_DIR}")

    # Load configs
    config_path = EXPERIMENT_DIR / "configs" / "round_01" / "r1_all_configs.json"
    ...
    # Load input PDB
    input_pdb = INPUT_DIR / "coordinating_motif.pdb"
    ...
    all_results = {}
    total_saved = 0
    n_designs = 4  # Start with 4 per config for faster iteration
    ...
    for config_name, config in configs.items():
        ...
        result = run_rfd3(config, pdb_content, config_name, n_designs=n_designs)
    ...
    summary = {
        "round": 1,
        "timestamp": datetime.now().isoformat(),
        "n_designs_per_config": n_designs,
    ...
    print(f"\nNext: Analyze with analyze_round.py --round 1")
